...
=== Spring Boot 4 and JUnit 6 Compatibility

Spring Boot 4 introduces pure JUnit 6 extensions
(for example `@EnableWireMock` from wiremock-spring-boot)
that rely on the JUnit Platform extension lifecycle.

Spock does not automatically execute arbitrary JUnit 6 extensions.
As a result, field-level injection mechanisms such as
`@InjectWireMock` may not work as expected in Spock specifications.

To enable full compatibility, add:
