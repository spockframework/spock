...
This report can be enabled according to the <<spock-configuration-file,Spock Configuration File>> section. The default is to not generate this
report.
...
NOTE: Build tools might trigger repeated executions, so those methods might be called more often than you might expect.
