...
Sometimes, you want to assert the contents of a collection regardless of its order.
...
* `x =~ [1, 2, 3]` is a lenient match, i.e., casting both to `Set` before comparing.
* `x ==~ [1, 2, 3, 3]` is a strict match, using Hamcrest's `containsInAnyOrder` under the hood so that duplicate elements are verified as well.
