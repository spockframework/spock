...
----
include::{sourcedir}/extension/FancyMockMaker.java[tag=fancy-mock-maker]
----

.Custom Mock Maker Usage Example
[source,groovy,indent=0]
----
include::{sourcedir}/extension/FancyMockMakerDocSpec.groovy[tag=fancy-mock-maker-usage]
----
...
The store is available from the `org.spockframework.runtime.extension.IMethodInvocation` interface via the `getStore(IStore.Namespace namespace)` method.
...
1. Root
2. Specification (shared)
3. Feature
4. Iteration

If an item is not found in the current store, the parent store is searched, and so on until the root store is reached. For more information see the Javadoc of the `org.spockframework.runtime.extension.IStore`.

NOTE: If an object stored in the store implements `java.lang.AutoCloseable`, then the `close()` method will be called when the store is closed.
