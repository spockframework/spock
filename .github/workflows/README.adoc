...
* If Kotlin is installed somewhere you can call the compiler with the `*.main.kts` script
  as argument:
+
[source,bash]
----
$ path/to/kotlinc -script release.main.kts
----
...
There are currently two known caveats with the approach we follow.
