...
- `@Rollup` can now be used on feature and spec level to explicitly roll any feature where the reporting of single
  iterations is not wanted.

- The system property `spock.globalRollup` can be set to `true` to roll all features by default if not overwritten
  by explicit `@Unroll` annotations and thus reinstate the pre Spock 2.0 behaviour.
