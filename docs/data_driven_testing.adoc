...
Note that rolling has no effect on how the method gets executed; it is only an alternation in reporting.
...
Alternatively the system property `spock.globalRollup` can be set to `true` to roll all features automatically unless
