...
public class TimeoutProcessor extends AbstractDirectiveProcessor {
  public void visitSpeckDirective(Annotation directive, SpeckInfo speck) {
    for (MethodInfo feature : speck.getFeatureMethods())
      if (!feature.getReflection().isAnnotationPresent(Timeout.class))
        feature.addInterceptor(new TimeoutInterceptor((Timeout)directive));
  }

  public void visitMethodDirective(Annotation directive, MethodInfo method) {
    method.addInterceptor(new TimeoutInterceptor((Timeout)directive));
  }
}