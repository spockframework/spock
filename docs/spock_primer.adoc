...
Interactions and Interaction based testing are described in a separate <<interaction_based_testing.adoc#_interaction_based_testing,chapter>>, so we only give a quick example here.
...
The `where` block is further explained in the <<data_driven_testing.adoc#_data_driven_testing,Data Driven Testing>> chapter.
...
Go to the <<extensions.adoc#_extensions_chapter,Extensions>> chapter to learn how to implement your own directives and extensions.
