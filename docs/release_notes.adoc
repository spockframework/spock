...
* `IResponseGenerator#respond` no longer returns `Object` but `Supplier<Object>`. To get the actual object you now
  have to additionally call `get()` on the supplied response, for example when you are using `EmptyOrDummyResponse`
  somewhere directly. On the other hand, if you are implementing a custom `IResponseGenerator`, you have go change
  your implementation accordingly. spockPull:1910[]
