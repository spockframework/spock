...
https://docs.oracle.com/javase/7/docs/api/java/lang/reflect/Proxy.html[JDK dynamic proxies] (when mocking interfaces)
and https://bytebuddy.net/[Byte Buddy] or https://github.com/cglib/cglib[CGLIB] proxies (when mocking classes) to generate mock implementations at runtime.
...
The Java world has no shortage of popular and mature mocking frameworks: https://www.jmock.org/[JMock],
https://www.easymock.org[EasyMock], https://mockito.org/[Mockito], to name just a few.
...
different <<extensions.adoc#mock-makers,mock makers>>.
...
The different <<extensions.adoc#mock-makers,mock makers>> have different capabilities and drawbacks.
...
[source,groovy,indent=0]
----
include::{sourcedir}/interaction/MockMakerDocSpec.groovy[tag=mock1]
----
...
include::{sourcedir}/interaction/BuilderExampleSpec.groovy[tag=builder]
----
...
[source,groovy,indent=0]
----
include::{sourcedir}/interaction/InteractionDocSpec.groovy[tag=spy1]
----
...
* `Mock/Stub/SpyStatic` static mocks: Works with Java and Groovy, but requires a mock maker supporting this, e.g. <<extensions.adoc#mock-makers-mockito,mockito>> Mock Maker.
...
* `MockStatic()`: Mocks static methods of the given type; supports both stubbing and mocking.
* `StubStatic()`: Mocks static methods of the given type; supports stubbing but not mocking.
...
NOTE: Global mocks will only work for Groovy code under test not for Java code.
...
https://www.ccs.neu.edu/research/demeter/related-work/extreme-programming/MockObjectsFinal.PDF[Endo-Testing: Unit Testing with Mock Objects]::
  Paper from the XP2000 conference that introduces the concept of mock objects.

https://www.jmock.org/oopsla2004.pdf[Mock Roles, not Objects]::
...
https://www.growing-object-oriented-software.com[Growing Object-Oriented Software Guided by Tests]::
