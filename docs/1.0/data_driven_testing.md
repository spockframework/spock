...
out that it’s the second iteration that failed. At other times this can be more difficult or even impossible.
 (Note: For example, a feature method could use data variables in its `setup:` block, but not in any conditions.)
...
Now we can tell at a glance that the `max` method failed for inputs `7` and `0`. See [_more_on_unrolled_method_names](#_more_on_unrolled_method_names)
