...
> [!NOTE]
> This section only touches on the breaking changes, see the [Release Notes](release_notes.md#release-notes) for a full list of changes and new features.
...
You can replace the `TemporaryFolder` rule with the new built-in `@TempDir` [extension](extensions.md#temp-dir).
