...
arguments as usual using `as`. If the testee for example has one constructor with a
`String` parameter and one with a `Pattern` parameter and you want `null` as
`constructorArg`:
