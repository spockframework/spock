...
* Add `globalTimeout` to  `@Timeout` extension, to apply a timeout to all features in a specification, configurable via the spock configuration file spockPull:1985[]
* Improve `@Timeout` extension will now use virtual threads if available spockPull:1985[]
