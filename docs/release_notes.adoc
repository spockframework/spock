...
With the new behavior, you can have a `Mock` or `Spy` return the same value as a `Stub`.
This is also more in line with the fact that `_` in arguments constraints stands for any non-`null` value, while a `Mock` returns `null` by default.
