...
While unrolling is not enabled by default, there is a 3rd-party global extension
https://github.com/szpak/spock-global-unroll[spock-global-unroll] which seamlessly enables it in the whole project
just by putting a JAR on a project classpath.
