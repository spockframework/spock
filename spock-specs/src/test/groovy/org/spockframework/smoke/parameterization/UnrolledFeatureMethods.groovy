...
def foo() {
  ...
  def "iterations of an unrolled feature foo are named 'foo [x: 1, iterationIndex: 0]', 'foo [x: 2, iterationIndex: 1]', etc."() {
    when:
    def result = runner.runSpecBody """
@Unroll
def foo() {
  ...
  x << [1, 2, 3]
    ...
    then:
    result.tests().started().list().testDescriptor.displayName == (0..2).collect {"foo [x: ${it + 1}, iterationIndex: $it]" }
  }
...
def foo() {
  ...
  def "naming pattern may refer to deprecated iteration count but print a warning"() {
    given:
...
def foo() {
  ...
  y << ["a", "b", "c"]
    ...
    then:
    result.tests().started().list().testDescriptor.displayName == ["one 0 two",
                                                                   "one 1 two",
                                                                   "one 2 two"]

    and:
    err.toString().contains('WARNING: #iterationCount in unroll pattern is deprecated, use #iterationIndex instead')
