...
* `SingleResponseGenerator#isAtEndOfCycle` is now `final` and `SingleResponseGenerator#doRespond` is now `protected`.
  When subclassing `SingleResponseGenerator` it does not make sense to override the first method, and it does not make
  sense to call the second method from somewhere else. This should not affect most users, only if you were
  subclassing `SingleResponseGenerator` and doing unusual things. spockPull:1910[]
