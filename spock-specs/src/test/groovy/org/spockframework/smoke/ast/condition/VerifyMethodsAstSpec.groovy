/*
 * Copyright 2025 the original author or authors.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *     https://www.apache.org/licenses/LICENSE-2.0
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package org.spockframework.smoke.ast.condition

import org.spockframework.EmbeddedSpecification
import org.spockframework.specs.extension.SpockSnapshotter
import spock.lang.Snapshot

class VerifyMethodsAstSpec extends EmbeddedSpecification {

  @Snapshot(extension = 'groovy')
  SpockSnapshotter snapshotter

  def "transforms all conditions in @Verify methods inside spec classes"() {
    given:
    snapshotter.specBody()

    when:
    def result = compiler.transpileSpecBody('''
@spock.lang.Verify
void isPositiveAndEven(int a) {
    a > 0
    a % 2 == 0
}
''')

    then:
    snapshotter.assertThat(result.source).matchesSnapshot()
  }

  def "transforms all conditions in @Verify methods outside of spec classes"() {
    when:
    def result = compiler.transpile('''
class Assertions {
    @spock.lang.Verify
    void isPositiveAndEven(int a) {
        a > 0
        a % 2 == 0
    }
}
''')

    then:
    snapshotter.assertThat(result.source).matchesSnapshot()
  }

  def "transforms conditions in private @Verify methods"() {
    given:
    snapshotter.specBody()

    when:
    def result = compiler.transpileSpecBody('''
@spock.lang.Verify
private void isPositive(int a) {
    a > 0
}
''')

    then:
    snapshotter.assertThat(result.source).matchesSnapshot()
  }

  def "transforms conditions in static @Verify methods"() {
    when:
    def result = compiler.transpile('''
class Assertions {
    @spock.lang.Verify
    static void isPositive(int a) {
        a > 0
    }
}
''')

    then:
    snapshotter.assertThat(result.source).matchesSnapshot()
  }

  def "transforms explicit conditions"() {
    when:
    def result = compiler.transpile('''
class Assertions {
    @spock.lang.Verify
    static void isPositiveAndEven(int a) {
        assert a > 0
        assert a % 2 == 0
    }
}
''')

    then:
    snapshotter.assertThat(result.source).matchesSnapshot()
  }

  def "methods without condition declarations stay unchanged"() {
    when:
    def result = compiler.transpile('''
class NonAssertions {
    @spock.lang.Verify
    static void copy(Integer a) {
        def uselessCopy = a
    }
}
''')

    then:
    snapshotter.assertThat(result.source).matchesSnapshot()
  }

  def "ignores methods without @Verify annotation"() {
    when:
    def result = compiler.transpile('''
class Assertions {
    @spock.lang.Verify
    static void isPositive(int a) {
        a > 0
    }

    static void nonAssertion() {
        println("nonAssertion")
    }
}
''')

    then:
    snapshotter.assertThat(result.source).matchesSnapshot()
  }

  def "transforms nested conditions"() {
    when:
    def result = compiler.transpile('''
class Assertions {
    @spock.lang.Verify
    static void brokenEvenIfPositive(int a) {
        a != 0
        if (a > 0) {
            a % 2 == 0
        }
    }
}
''')

    then:
    snapshotter.assertThat(result.source).matchesSnapshot()
  }

  def "checks for invalid conditions"() {
    when:
    def result =  compiler.transpile('''
class Assertions {
    @spock.lang.Verify
    static void assignmentNotCondition(int a) {
        a = 3
    }
}
''')

    then:
    result.source.contains("Expected a condition, but found an assignment. Did you intend to write '==' ? @ line 5, column 9")
  }

  def "ignores conditions in overwritten @Verify methods"() {
    when:
    def result = compiler.transpile('''
class BaseAssertions {
    @spock.lang.Verify
    void isValid(int a) {
        a >= 0
    }
}

class SpecificAssertions extends BaseAssertions {
    @Override
    void isValid(int a) {
        a > 0
    }
}
''')

    then:
    snapshotter.assertThat(result.source).matchesSnapshot()
  }

  def "ignores when transforming methods with non-void return value"() {
    when:
    def result = compiler.transpile('''
class Assertions {
    @spock.lang.Verify
    static boolean isPositiveWithReturn(int a) {
        a > 0
        return true
    }
}
''')

    then:
    snapshotter.assertThat(result.source).matchesSnapshot()
  }

}
