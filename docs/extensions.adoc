...
`@Retry` annotations are inherited, i.e. if declared on a superclass of a spec class, it will be applied to all feature
methods of both classes unless the subclass declares its own annotation. If so, it is applied to all feature methods
declared in the subclass, but not to any feature methods declared in the superclass. In particular, that means that
retries are not applied to inherited feature methods if the annotation is only present on the subclass.

Given the following example, running `FooIntegrationSpec` will execute both `inherited` and `foo` with one retry.
Running `BarIntegrationSpec` will execute `inherited` with one and `bar` with two retries.
