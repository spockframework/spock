...
class ClosureBasedUnrollNameProviderSpec extends Specification {
  ...
  def "regex-like data values are substituted correctly (i.e. literally)"() {
    ...
    def nameProvider = new ClosureBasedUnrollNameProvider(feature, {"foo $dataVar bar"}.class)

    expect:
    nameProvider.nameFor(value) == name
