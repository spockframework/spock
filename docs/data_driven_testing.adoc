...
Sometimes you need a helper value to build your data, but you do not want it to become a data variable (and therefore a method parameter) or a field on the whole specification.
...
The automatic close is best-effort and has two known limitations.
If one initializer throws after an earlier variable was already created, that earlier resource is *not* closed, because the where-block variables are computed together as a single unit and a failed computation produces no values to close.
...
NOTE: The `final (a, b) = ...` is only supported for Groovy 3.0 or newer.
