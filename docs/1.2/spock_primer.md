...
Interactions and Interaction based testing are described in a separate [chapter](interaction_based_testing.md), so we only give a quick example here.
...
The `where` block is further explained in the [Data Driven Testing](data_driven_testing.md) chapter.
...
Go to the [Extensions](extensions.md) chapter to learn how to implement your own directives and extensions.
