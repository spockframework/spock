...
`TempDir` can be applied to annotate a member field of type `java.io.File` or `java.nio.file.Path`
in a spec class. If the annotated field is shared, the temp directory will be shared in this spec, otherwise every
iteration will have its own temp directory:

[source,groovy]
----
include::{sourcedir}/extension/TempDirDocSpec.groovy[tag=example-common]
