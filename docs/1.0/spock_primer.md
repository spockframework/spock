...
Let’s start with a few definitions: Spock lets you write *specifications* that describe expected *features* (properties,
aspects) exhibited by a system of interest. The system of interest could be anything between a single class and a whole
application, and is also called *system under specification (SUS)*. The description of a feature starts from a specific
snapshot of the SUS and its collaborators; this snapshot is called the feature’s *fixture*.
...
optional and may be omitted, resulting in an *implicit* `setup` block. The `given:` label is an alias for `setup:`,
and sometimes leads to a more readable feature method description (see [Specifications as Documentation](#specs-as-doc)).
...
Interactions are devoted a whole [chapter](interaction_based_testing.md), and so we only give a quick example here.
...
The `where` block will be further explained in the [Data Driven Testing](data_driven_testing.md) chapter.
...
To learn how to implement your own directives and extensions, go to the [Extensions](extensions.md) chapter.
