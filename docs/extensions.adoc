...
When a timeout is reported to the user, the test output contains a thread dump reflecting the execution stack of the test framework when the timeout was exceeded. By default, the thread dump is also captured on all unsuccessful interrupt attempts.

If you want to customize the thread dump capturing behavior, you can use the <<spock-configuration-file,Spock Configuration File>>.
