...
public class RollupExtension extends AbstractAnnotationDrivenExtension<Rollup> {
  ...
  public void visitSpecAnnotation(Rollup rollup, SpecInfo spec) {
    visitSpecAnnotation(rollup, spec, true);
  }

  private void visitSpecAnnotation(Rollup rollup, SpecInfo spec, boolean direct) {
    ...
    for (FeatureInfo feature : spec.getFeatures()) {
      if (feature.isParameterized()) {
        visitFeatureAnnotation(rollup, feature, false);
      }
    }
  }
  ...
  public void visitFeatureAnnotation(Rollup rollup, FeatureInfo feature) {
    visitFeatureAnnotation(rollup, feature, true);
  }

  private void visitFeatureAnnotation(Rollup rollup, FeatureInfo feature, boolean direct) {
  ...
  public static class AutoApply extends AbstractGlobalExtension {
    ...
    private final Rollup rollup = Rolled.class.getAnnotation(Rollup.class);

    @Override
    public void visitSpec(SpecInfo spec) {
      if (globalRollup && spec.getAnnotation(Unroll.class) == null) {
        new RollupExtension().visitSpecAnnotation(rollup, spec, false);
      }
    }

    @Rollup
    private static final class Rolled {
    }
  }
}
