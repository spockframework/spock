...
.Store hierarchy
[plantuml,images/extension-store,svg]
....
@startuml
state Root #88CCEE {
    state ASpec #DDCC77 {
...
[source,groovy,indent=0]
----
include::{sourcedir}/extension/ExtensionStoreSpec.groovy[tag=example-extension]
----
...
<8> It then adds counts the occurred exception type, creating the counter if it was not yet present.
...
[source,groovy,indent=0]
----
include::{sourcedir}/extension/ExtensionStoreSpec.groovy[tag=example-spec]
----
