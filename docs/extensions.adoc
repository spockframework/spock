...
When a timeout is reported to the user, the test output contains thread dumps reflecting the execution stack of the test framework when the timeout was exceeded. By default, thread dumps are also captured on all unsuccessful interrupt attempts.
