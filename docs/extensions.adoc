...
or untyped using `def` in a spec class. If the annotated field is shared,
the temp directory will be shared in this spec, otherwise every
iteration will have its own temp directory:

[source,groovy,indent=0]
----
include::{sourcedir}/extension/TempDirDocSpec.groovy[tag=example-a]
----

To configure where the temp dirs are created and
whether they should survive for investigation,
`baseDir` and `keep` can be provided:

[source,groovy,indent=0]
----
include::{sourcedir}/extension/TempDirDocSpec.groovy[tag=example-b]
----

Param `keep` is a delegate of type `org.spockframework.runtime.extension.builtin.PreconditionContext`,
which works exactly like param of `IgnoreIf`.
...
Spock can generate a temp directory for test and delete it after test,
which is related with <<TempDir,`@TempDir`>>.

.Temp Directory Configuration
[source,groovy]
----
tempdir {
    baseDir false // default null, which means System.getProperty("java.io.tmpdir")
    keep { os.windows } // default { false }, which is called with a delegate of type PreconditionContext
}
----
