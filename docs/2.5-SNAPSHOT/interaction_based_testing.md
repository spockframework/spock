...
block. (Fans of [Data Driven Testing](data_driven_testing.md#data-driven-testing) might move the variable into a `where:` block.)
...
different [mock makers](extensions.md#mock-makers).
...
The different [mock makers](extensions.md#mock-makers) have different capabilities and drawbacks.
...
- `SpyStatic()` static mocks: Works with Java and Groovy, but requires a mock maker supporting this, e.g. [mockito](extensions.md#mock-makers-mockito) Mock Maker.
...
> [!NOTE]
> The static mocks require a mock maker supporting static methods, e.g. [mockito](extensions.md#mock-makers-mockito).
> See [mock makers](extensions.md#mock-makers) table for mock makers supporting it.
...
requires that the spec is annotated with [@Isolated](parallel_execution.md#isolated-execution) or `@ResourceLock(org.spockframework.runtime.model.parallel.Resources.META_CLASS_REGISTRY)`.
...
>       This either happens manually by calling `MockUtil.attachMock(Object, Specification)` or automatically by declaring the mock as an instance field with an `@AutoAttach` annotation, see [AutoAttach](extensions.md#_autoattach).
