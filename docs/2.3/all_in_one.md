...
[Spock Web Console](https://meetspock.appspot.com/) is a website that allows you to instantly view, edit, run, and
even publish Spock specifications. It is the perfect place to toy around with Spock without making any commitments.
So why not run [Hello, Spock!](https://meetspock.appspot.com/edit/9001) right away?
...
> [!NOTE]
> This will also set the execution mode to `SAME_THREAD`, see [Parallel Execution](parallel-execution.md#parallel-execution) for more information.
...
> [!NOTE]
> This will also set the execution mode to `SAME_THREAD` if applied on a `Specification`, see [Parallel Execution](parallel-execution.md#parallel-execution) for more information.
...
> [!NOTE]
> This will acquire a `READ_WRITE` lock for `Resources.META_CLASS_REGISTRY`, see [Parallel Execution](parallel-execution.md#parallel-execution) for more information.
...
> [!NOTE]
> This will acquire a `READ_WRITE` lock for `Resources.SYSTEM_PROPERTIES`, see [Parallel Execution](parallel-execution.md#parallel-execution) for more information.
...
For trivial cases like creating a single temp directory, you can use the [@TempDir](#_temp_dir) extension directly.
...
- Add RepeatUntilFailure extension [#1522](null/pull/1522)
- Fix problem with TempDir failing when using a custom class that extends from `File` or `Path` [#1519](null/pull/1519)
- Fix issue with `Object` methods on interface spies [#1529](null/pull/1529)
- Fix equality checking of Mocks that implement `Comparable` [#1323](null/pull/1323)
- Validate uniqueness of fixture methods per specification class [#1521](null/pull/1521)
...
- Support for `@SpringBean` and `@SpringSpy` marked with `@Primary` [#1503](null/pull/1503)
- Fix issue that `displayName` values for the `Specification`, which were changed by extensions such as [@Title](#title-and-narrative-extension), not being picked up. [#1509](null/pull/1509)
...
- Add junit-platform `TestTag` support with the [@Tag](#test-tag-extension) extension [#1467](null/pull/1467)
- Add `IDataDriver` extension point and refactor data provider handling [#1479](null/pull/1479)
- Add [named deconstruction](data-driven-testing.md#multi-data-pipe-named) for multi variable datapipes.
This might lead to changed behavior results if the data object implements `Map` but supports both positional `getAt(int)` and named `getAt(String)` access. In earlier versions, the positional access was used, but now the named access will be used. [#1463](null/pull/1463)
- Add custom class support to `@TempDir`, you can use any class that has a single `java.io.File` or `java.nio.file.Path` as constructor parameter [#1430](null/pull/1430)
- Improve `@Stepwise` can be applied to data-driven feature methods, having the effect of executing them sequentially (even if concurrent test mode is active) and to skip subsequent iterations is one iteration fails. [#1442](null/pull/1442)
- Improve `EmbeddedSpecCompiler` by making package declaration configurable in `EmbeddedSpecCompiler`
- Improve do not evaluate feature-skipping conditions for skipped specs [#1459](null/pull/1459)
- Improve behavior when trying to run Spock with unsupported Groovy version in IDEA [#1421](null/issues/1421)
- Improve Java 17 compatibility by using new `invokeDefault` instead of reflection
- Fix compatibility with certain locales, for example `tr_TR` [#1414](null/issues/1414)
- Fix Spring 6 incompatibility [#1428](null/issues/1428)
- Fix test reporting issue with Maven, where an error on the Specification level was not visible [#1444](null/issues/1444)
- Fix context pollution in `IterationNode.around()` [#1441](null/issues/1441)
- Fix Discard unnecessary state in `ConfineMetaClassChangesInterceptor` [#1460](null/pull/1460)
- Fix make `EnableSharedInjection` public [#1472](null/pull/1472)
- Fix gradle module metadata dependencies [#1490](null/pull/1490)
- Remove runtime dependency on Jetbrains Annotations [#1468](null/pull/1468)
...
- Add Groovy 4 support [#1382](null/pull/1382)
...
- Add [collection conditions](#collection-conditions) [#1372](null/issues/1372)
- Add support for selecting individual iterations via their unique ID (IDE support required). [#1376](null/pull/1376)


#### Breaking Changes

- Add `data.` support to conditional extensions [#1360](null/issues/1360).
This replaces the  current behavior of accessing data variables without any prefix.
See [Precondition Context](#precondition_context) for more details.


#### Misc

- Add exception translation to JUnit4 Rules [#1342](null/pull/1342)
- Add option to omit feature name from iterations [#1386](null/pull/1386)
and add additional [Special Tokens](#unroll_tokens) to unroll patterns.
- Add optional reason to `@Requires` and `@IgnoreIf` [#1362](null/pull/1362)
- Add `shared.` support to conditional extensions [#1359](null/pull/1359).
See [Precondition Context](#precondition_context) for more details.
- Set the owner for condition closures on spec annotations [#1357](null/pull/1357)
  This allows access to static methods when `@IgnoreIf`, `@Requires` and `@PendingFeatureIf` is used on a Specification.
  
  
- Update bnd Gradle plugin to 6.0.0 (OSGI) [#1377](null/pull/1377)
- Fix selector ordering issue [#1375](null/pull/1375)
- Fix `@TempDir` not working for `@Shared` inherited fields [#1373](null/pull/1373)
- Fix JUnit rule run order [#1363](null/pull/1363)
- Prevent removal of ErrorSpecNode from execution hierarchy [#1358](null/pull/1358)
...
- Fix double invocation of IRunListener.beforeSpec and IRunListener.afterSpec [#1344](null/pull/1344)
- Fix regression with multi-assignment of method call result [#1333](null/pull/1333)
- Fix Build MethodSource with the correct spec class name [#1345](null/pull/1345)
...
- Fix NPE if variable is initialized using a method with the same in features with cleanup blocks or using thrown condition [#1266](null/issues/1266) [#1313](null/issues/1313)
- Fix implicit this conversion after bugfix in Groovy 3.0.8 [#1318](null/issues/1318)
- Fix extension-provided method arguments in fixture methods [#1305](null/issues/1305)
...
`@EnableSharedInjection` to the specification. [#76](null/issues/76)
...
unmodifiable to user code. [#1011](null/issues/1011)
- Improve parallel extensions to support inheritance [#1245](null/issues/1245)
...
- Fix [#1294](null/issues/1294) swallowing of unrecoverable Errors
- Fix [#1260](null/issues/1260) Change InteractionRewriter to keep casting of ListExpressions intact
- Fix [#1282](null/issues/1282) Make TempDirInterceptor safe for parallel invocation of iterations
- Fix [#1267](null/issues/1267) Retry extension behavior for unrolled tests
- Fix [#1267](null/issues/1267) Retry extension behavior for unrolled tests
- Fix [#1229](null/issues/1229) `@TempDir` not working for inherited fields
- Fix [#1232](null/issues/1232) compile error for nested conditions without top-level condition
- Fix [#1256](null/issues/1256) handling of `is` as getter for boolean properties on Mocks
- Fix [#1270](null/issues/1270) handling of `is` as getter for boolean properties on GroovyMocks
- Fix `ErrorSpecNode` to re-throw `Exception` in `prepare` instead of `execute`,
this fixes an issue that interceptors for `prepare`, `before`, and `around`
were still executed and any Exception they throw would hide the actual cause.
- Fix [#1263](null/issues/1263) ExceptionAdapterExtension to also handle inherited fixture methods
- Fix [#1279](null/issues/1279) Cast data variables with type coercion to the declared parameter type
...
- Fix [#1158](null/issues/1158) Fix strange bug with setter/getter handling of mocks in groovy
- Fix [#1216](null/issues/1216) perform argument coercion for `GroovyMock` method arguments
- Fix [#1169](null/issues/1169) check skipped state in Node.prepare and do nothing if already skipped
- Fix [#1200](null/issues/1200) name clashes where variables that are named like method calls destroy the method call
- Fix [#1202](null/issues/1202) NullPointerException with array initializers
- Fix [#994](null/issues/994) don’t treat nested closures in argument constraints as implicit assertions anymore
...
For more information see issue [#1022](null/issues/1022).
...
- Forbid spying on `Spy` instances, as this doesn’t work anyway and leads to wrong expectations ([#1029](null/issues/1029))
...
- Add support for Java 11+ ([#895](null/issues/895), [#902](null/issues/902), [#903](null/issues/903))
...
- Add flag to UnrollNameProvider to assert unroll expressions (set the system property `spock.assertUnrollExpressions` to `true`) ([#767](null/issues/767))
...
- Improve `@PendingFeature` to now have an optional `reason` attribute ([#907](null/issues/907))
...
- Improve StepwiseExtension mark only subsequent features as skipped in case of failure ([#893](null/issues/893))
...
- Fix use String renderer for Class instances ([#909](null/issues/909))
- Fix mark new Spring extensions as @Beta ([#890](null/issues/890))
- Fix exclude groovy-groovysh from compile dependencies ([#882](null/issues/882))
- Fix `Retry.Mode.FEATURE` and `Retry.Mode.SETUP_FEATURE_CLEANUP` to make a test pass if a retry was successful.
- Fix issue with `@SpringBean` mocks throwing `InvocationTargetException` instead of actual declared exceptions ([#878](null/issues/878), [#887](null/issues/887))
- Fix void methods with implicit targets failing in `with` and `verifyAll` ([#886](null/issues/886))
...
- Fix re-declare recorder variables ([#783](null/issues/783)), this caused annotations such as `@Slf4j` to break Specifications
- Fix MissingFieldException in DiffedObjectAsBeanRenderer
- Fix problems with nested `with` and `verifyAll` method calls
- Fix assertion of mock invocation order with nested invocations ([#475](null/issues/475))
...
(see [spock-example](https://examples.spockframework.org) project).
...
[Spock Example](https://github.spockframework.org/spock-example) project, and
...
> [!NOTE]
> This section only touches on the breaking changes, see the [Release Notes](#_release_notes) for a full list of changes and new features.
...
### Usage of `final` in a feature method with `cleanup` [#1017](null/issues/1017)
