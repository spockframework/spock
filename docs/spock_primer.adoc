...
When factoring out conditions into a helper method, you need to ensure the method has return type of `void`,
or uses a dynamic return type ('def' or no declared type). After compilation it will in both cases be `void` and any
other return type causes a compilation error, as Spock might interpret the return value as a failing condition otherwise.
