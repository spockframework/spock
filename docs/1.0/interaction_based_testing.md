...
and [CGLIB](https://github.com/cglib/cglib) proxies (when mocking classes) to generate mock implementations at runtime.
...
block. (Fans of [Data Driven Testing](data_driven_testing.md) might move the variable into a `where:` block.)
...
> [!NOTE]
> Java 8 is only supported from CGLIB 3.2 onwards, which has not been released as of
