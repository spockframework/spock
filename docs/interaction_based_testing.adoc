...
just like mocking interfaces; the only additional requirement is to put `byte-buddy` or
`cglib-nodep-2.2` or higher on the class path. When using normal `Mock`s or `Stub`s or `Spy`s that are
...
NOTE: Java 8 is only supported with Byte Buddy or CGLIB from 3.2.0 onwards.
